...
static void aarch64_tr_tb_start(DisasContextBase *db, CPUState *cpu)
{
    ...
    if (x_ray_is_switch_addr (pc_arg)) {
        gen_helper_aarch64_update_current_task (tcg_env);
    }
    if (x_ray_need_insert_hook (cpu, pc_arg)) {
        printf ("inserting hook..\n");
        TCGv_i64 pc = tcg_constant_i64 (pc_arg);
        gen_helper_x_ray_insert_hook (tcg_env, pc);
    }
    #endif
}
